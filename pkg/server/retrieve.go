...
func (s *retrieveServlet) retrieve(w http.ResponseWriter, r *http.Request) result {
	...
	//	return s.fail(log(ctx, err), w, "request for non-current date", "only serving current-period data for pilot study", http.StatusNotFound)
	//}
	//} else 
	if dateNumber > currentDateNumber {
	...
	log(ctx, nil).WithField("dateNumber", dateNumber).WithField("currentDateNumber", currentDateNumber).WithField("unzipped-size", size).WithField("keys", len(keys)).Info("Wrote retrieval")
	return result(struct{}{})
}
